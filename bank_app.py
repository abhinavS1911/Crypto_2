...
def write_blockchain_to_file():
    """Request blockchain from bank server and write to JSON file"""
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((BANK_HOST, BANK_PORT))
        s.sendall(json.dumps({"type": "get_blockchain"}).encode())
        response = s.recv(100000)
        data = json.loads(response.decode())
        if data.get("status") == "success":
            with open(BLOCKCHAIN_FILE, "w") as f:
                json.dump(data["chain"], f, indent=4)
...
def send_request(data, host, port):
    try:
        ...
        # Check if action caused blockchain update
